...
public class Peaklass {
    ...
    public static void main(String[] args) {
        ...
        while (kestab) {
            ...
            double algus = portfell.arvutaVäärtus(aktsia.getHind()); // salvestame väärtuse enne simulatsiooni
            String tekst = "";

            for (int indeks = 1; indeks <= päevi; indeks++) { // tsükkel käib läbi ennustuse simulatsiooni päevad
                aktsia.uuendaHinda(turu.randomMuutus(aktsia.getVolatiilsus())); // tuleb juhuslik protsent ning sellega uuendataks hinda
                if (päevi <= 5 || indeks % 5 == 0 || indeks == päevi) {
                    tekst += "Päev " + indeks + ": " + Math.round(aktsia.getHind() * 100.0) / 100.0 + " EUR\n";
                }
            }

            double lõpp = portfell.arvutaVäärtus(aktsia.getHind()); // portfelli väärtus pärast muutust
